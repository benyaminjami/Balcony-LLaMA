...
@add_start_docstrings(
    """
    The NestedLlama Model transformer with a sequence classification head on top (linear layer).

    [`NestedLlamaForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
...
class NestedLlamaForSequenceClassification(NestedLlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
@add_start_docstrings(
    """
The NestedLlama Model transformer with a span classification head on top for extractive question-answering tasks like
SQuAD (a linear layer on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    NestedLlama_START_DOCSTRING,
)
class NestedLlamaForQuestionAnswering(NestedLlamaPreTrainedModel):
    base_model_prefix = "transformer"

    # Copied from transformers.models.bloom.modeling_bloom.BloomForQuestionAnswering.__init__ with Bloom->NestedLlama
    def __init__(self, config):
        super().__init__(config)
        self.transformer = NestedLlamaModel(config)
...
@add_start_docstrings(
    """
    The NestedLlama Model transformer with a token classification head on top (a linear layer on top of the hidden-states
    output) e.g. for Named-Entity-Recognition (NER) tasks.
    """,
    NestedLlama_START_DOCSTRING,
)
class NestedLlamaForTokenClassification(NestedLlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = NestedLlamaModel(config)
...
__all__ = [
    ...
    "NestedLlamaModel",
    "NestedLlamaPreTrainedModel",
    "NestedLlamaForSequenceClassification",
    "NestedLlamaForQuestionAnswering",
    "NestedLlamaForTokenClassification",
]