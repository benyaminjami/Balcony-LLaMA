...
class _ShardedKLDivergence(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        sharded_logits_q,  # (batch_size, length, sharded_hidden_size)
        sharded_logits_p,  # (batch_size, length, sharded_hidden_size)
        group: dist.ProcessGroup,
    ):
        # Maximum value along the last dimension across all GPUs for numerical stability.
        logits_max_q = torch.max(sharded_logits_q, dim=-1)[0]
        logits_max_p = torch.max(sharded_logits_p, dim=-1)[0]

        dist.all_reduce(logits_max_q, op=dist.ReduceOp.MAX, group=group)
        dist.all_reduce(logits_max_p, op=dist.ReduceOp.MAX, group=group)

        # Subtract the maximum value for numerical stability.
        sharded_logits_q = sharded_logits_q - logits_max_q.unsqueeze(dim=-1)
        sharded_logits_p = sharded_logits_p - logits_max_p.unsqueeze(dim=-1)

        # Compute exp(logits) for both distributions.
        exp_logits_q = sharded_logits_q.exp()
        exp_logits_p = sharded_logits_p.exp()

        # Sum of exponential of logits along vocab dimension across all GPUs.
        sum_exp_logits_q = exp_logits_q.sum(dim=-1)
        sum_exp_logits_p = exp_logits_p.sum(dim=-1)

        dist.all_reduce(sum_exp_logits_q, op=dist.ReduceOp.SUM, group=group)
        dist.all_reduce(sum_exp_logits_p, op=dist.ReduceOp.SUM, group=group)

        # Compute probabilities.
        probs_q = exp_logits_q / sum_exp_logits_q.unsqueeze(dim=-1)
        probs_p = exp_logits_p / sum_exp_logits_p.unsqueeze(dim=-1)

        # Store probabilities for backward pass.
        ctx.save_for_backward(probs_q, probs_p)

        # Clamp probabilities to avoid log(0).
        epsilon = 1e-12  # Small value to avoid instability.
        probs_q = torch.clamp(probs_q, min=epsilon)
        probs_p = torch.clamp(probs_p, min=epsilon)
        # Compute KL divergence: KL(P || Q) = sum(P * log(P / Q)).
        kl_div = probs_p * (torch.log(probs_p) - torch.log(probs_q))

        # Sum over the last dimension to get scalar KL divergence per element.
    ...
    def backward(ctx, grad_output):
        # Retrieve tensors from the forward path.
        probs_q, probs_p = ctx.saved_tensors

        # Gradient with respect to logits_q.
        grad_logits_q = probs_q - probs_p

        # Elementwise multiplication with grad_output.
        grad_logits_q.mul_(grad_output.unsqueeze(dim=-1))

        return grad_logits_q, None, None


def shared_kl_divergence(sharded_logits, sharded_logits_teacher, group: dist.ProcessGroup, dtype: torch.dtype = None):
    """Helper function for the kl divergence."""
    if dtype is not None:
        # Cast input to specific dtype.
        sharded_logits = sharded_logits.to(dtype=dtype)
        sharded_logits_teacher = sharded_logits_teacher.to(dtype=dtype)
    return _ShardedKLDivergence.apply(sharded_logits, sharded_logits_teacher, group)
